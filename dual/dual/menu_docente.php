...
function esDocente($usuario) {
    ...
    $query = "SELECT * FROM docentes WHERE usuario = '$usuario'";
    $resultado = mysqli_query($conn, $query);
    $esDocente = mysqli_num_rows($resultado) > 0;
    mysqli_close($conn);
    return $esDocente;
}
...
$sql = "SELECT *, idDocente FROM docentes WHERE usuario = '$usuario'";
...
// Aquí agregamos el idDocente a la sesión
                        ...
                        <div class="container-fluid">
                            <div class="table-responsive text-center table-hover">
                                <table class="table table-responsive-sm text-center table-hover" id="tabla_id">
                                    ...
                                    <tbody>
                                        <?php
$docenteActual = $_SESSION['idDocente'];
include("templates/conexion.php");

// Construir la consulta principal utilizando el idAlumno y el idDocente actual
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          WHERE r.estatus_tutor = 'sin_revisar' 
          AND r.idAlumno IN (SELECT idAlumno FROM alumnos_docentes WHERE idDocente = '$docenteActual')
...
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          INNER JOIN tutores t ON r.idTutor = t.idTutor
          INNER JOIN instructores i ON r.idInstructor = i.idInstructor
          WHERE r.estatus_tutor = 'no_aprobado' 
          AND r.idAlumno IN (SELECT idAlumno FROM alumnos_docentes WHERE idDocente = '$docenteActual')
          ORDER BY r.idReporte DESC";
$respuesta = mysqli_query($conn, $query);
...
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          INNER JOIN tutores t ON r.idTutor = t.idTutor
          INNER JOIN instructores i ON r.idInstructor = i.idInstructor
          WHERE r.estatus_tutor = 'aprobado' 
          AND r.idAlumno IN (SELECT idAlumno FROM alumnos_docentes WHERE idDocente = '$docenteActual')
          ORDER BY r.idReporte DESC";
$respuesta = mysqli_query($conn, $query);
