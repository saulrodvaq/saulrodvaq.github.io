...
if(isset($_GET["idSubdirector"])) {
    // Obtener el valor del campo idSubdirector de la URL
    $idSubdirector = $_GET["idSubdirector"];

    // Consulta para obtener los datos del Subdirector correspondiente
    $query = "SELECT * FROM subdirectores WHERE idSubdirector = $idSubdirector";
    $resultado = mysqli_query($conn, $query);
    $subdirector = mysqli_fetch_assoc($resultado);
}
...
<head>
    ...
    <title>Editar Subdirectivos</title>
...
<body>
    ...
    <!-- Formulario para editar los datos del subdirector -->
    <div class="container">
        <br />
        <br />
        ...
        <form action="actualizar.php" method="POST">
            <div class="row">
                <div class="col-md-6 offset-md-3">
                    <input type="hidden" class="form-control mb-3" name="idSubdirector"
                        value="<?php echo $subdirector['idSubdirector']; ?>">
                    <label for="nombres">Tipo:</label>
                    <input type="text" class="form-control mb-3" name="tipo"
                        value="<?php echo $subdirector['tipo']; ?>">
                    <label for="nombres">Nombre(s):</label>
                    <input type="text" class="form-control mb-3" name="nombres"
                        value="<?php echo $subdirector['nombres']; ?>">
                    <label for="apellidos">Apellidos:</label>
                    <input type="text" class="form-control mb-3" name="apellidos"
                        value="<?php echo $subdirector['apellidos']; ?>">
                    <label for="matricula">Matrícula:</label>
                    <input type="text" class="form-control mb-3" name="matricula"
                        value="<?php echo $subdirector['matricula']; ?>">
                    <label for="carrera">Carrera:</label>
                    <select class="form-select mb-3" name="carrera" id="country">
                        <option>...</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'Ingeniería en Desarrollo y Gestión de Software') echo 'selected'; ?>>
                            Ingeniería en Desarrollo y Gestión de Software</option>
                        <option <?php if ($subdirector['carrera'] == 'Ingeniería en Mecatrónica') echo 'selected'; ?>>
                            Ingeniería en Mecatrónica</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'Ingeniería en Sistemas Productivos') echo 'selected'; ?>>
                            Ingeniería en Sistemas
                            Productivos</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'Ingeniería en Mantenimiento Industrial') echo 'selected'; ?>>
                            Ingeniería en Mantenimiento Industrial</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'Licenciatura en Innovación de Negocios y Mercadotecnia') echo 'selected'; ?>>
                            Licenciatura en Innovación de Negocios y Mercadotecnia</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'Licenciatura en Gestión Institucional Educativa y Curricular') echo 'selected'; ?>>
                            Licenciatura en Gestión Institucional Educativa y Curricular</option>
                        <option <?php if ($subdirector['carrera'] == 'TSU en Lengua Inglesa') echo 'selected'; ?>>
                            TSU en Lengua Inglesa</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'TSU en Tecnologías de la Información área Desarrollo de Software Multiplataforma') echo 'selected'; ?>>
                            TSU en Tecnologías de la Información área Desarrollo de Software Multiplataforma</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'TSU en Desarrollo de Negocios área Mercadotecnia') echo 'selected'; ?>>
                            TSU en Desarrollo de Negocios área Mercadotecnia</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'TSU en Mantenimiento área Industrial') echo 'selected'; ?>>
                            TSU en Mantenimiento área Industrial</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'TSU en Procesos Industriales área Industrial') echo 'selected'; ?>>
                            TSU en Procesos Industriales área Manufactura</option>
                        <option
                            <?php if ($subdirector['carrera'] == 'TSU en Mecatrónica área Automatización') echo 'selected'; ?>>
                            TSU en Mecatrónica área Automatización</option>
                    </select>
                    <label for="usuario">Usuario:</label>
                    <input type="text" class="form-control mb-3" name="usuario"
                        value="<?php echo $subdirector['usuario']; ?>">
                    <label for="password">Password:</label>
                    <input type="text" class="form-control mb-3" name="password"
                        value="<?php echo $subdirector['password']; ?>">
