...
if(isset($_GET["idTutor"])) {
    // Obtener el valor del campo idTutor de la URL
    $idTutor = $_GET["idTutor"];

    // Consulta para obtener los datos del Tutor correspondiente
    $query = "SELECT * FROM tutores WHERE idTutor = $idTutor";
    ...
    <header id="header" class="fixed-top2 header-inner-pages">
        <div class="container d-flex align-items-center justify-content-between">
            ...
            <nav id="navbar" class="navbar">
                <ul>
                    <li><a class="scrollto efectoboton" href="tutores.php">Regresar</a></li>
                </ul>
            ...
            <main class="container" style="max-width: 100% !important">
                <div class="row mb-2">
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                <h1 class="card-title text-center">Alumnos asignados al tutor</h1>
                                <table id="tabla-alumnos-asignados" class="display">
                                    ...
                                    <tbody>
                                        <?php
$query_alumnos_asignados = "SELECT * FROM alumnos WHERE idTutor = $idTutor";
...
while ($alumno_asignado = mysqli_fetch_assoc($resultado_alumnos_asignados)) {
    ...
    echo "<td><button class='btn btn-secondary btn-desasignar' data-id='" . $alumno_asignado['idAlumno'] . "' data-url='" . $_SERVER['PHP_SELF'] . "?idTutor=" . $idTutor . "'>Desasignar</button></td>";
    echo "</tr>";
}
?>
                                    </tbody>
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                <h1 class="card-title text-center">Alumnos sin tutor asignado</h1>
                                <table id="tabla-alumnos-sin-tutor" class="display">
                                    <thead>
                                        <tr>
                                            <th>ID</th>
                                            <th>Alumno</th>
                                    ...
                                    <tbody>
                                        <?php
$query_alumnos_sin_tutor = "SELECT * FROM alumnos WHERE idTutor IS NULL OR idTutor = 0";
$resultado_alumnos_sin_tutor = mysqli_query($conn, $query_alumnos_sin_tutor);

while ($alumno_sin_tutor = mysqli_fetch_assoc($resultado_alumnos_sin_tutor)) {
    echo "<tr>";
    echo "<td>" . $alumno_sin_tutor['idAlumno'] . "</td>";
    echo "<td>" . $alumno_sin_tutor['apellidos'] . ' ' . $alumno_sin_tutor['nombres'] . "</td>";
    echo "<td><button class='btn btn-primary btn-asignar' data-id='" . $alumno_sin_tutor['idAlumno'] . "' data-url='" . $_SERVER['PHP_SELF'] . "?idTutor=" . $idTutor . "'>Asignar</button></td>";
    echo "</tr>";
}
?>
                                    </tbody>
        ...
        $(document).ready(function() {
            // Configuración de las tablas de alumnos asignados al tutor
            ...
            // Configuración de las tablas de alumnos sin tutor asignado
            var tablaAlumnosSinTutor = $('#tabla-alumnos-sin-tutor').DataTable({
                "paging": true,
                "lengthChange": true,
                "lengthMenu": [5, 10, 15, 25, 50],
                "language": {
            ...
            $('#tabla-alumnos-sin-tutor').on('click', '.btn-asignar', function() {
                var idAlumno = $(this).data('id');
            ...
            function desasignarAlumno(idAlumno) {
                $.ajax({
                    url: 'desasignar_alumno.php', // Ruta del archivo PHP para desasignar alumno
                    method: 'POST',
                    data: {
                        idAlumno: idAlumno
                    },
            ...
            function asignarAlumno(idAlumno) {
                $.ajax({
                    url: 'asignar_alumno.php', // Ruta del archivo PHP para asignar alumno
                    method: 'POST',
                    data: {
                        idAlumno: idAlumno,
                        idTutor: <?php echo $idTutor; ?>
                    },
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                if (response.reload) {
                                    location
                                        .reload(); // Recargar la página si se debe hacer
                                } else {
                                    tablaAlumnosSinTutor.clear()
                                        .draw(); // Limpiar y dibujar la tabla de alumnos sin tutor
                                }
                            });
                        } else {
                            Swal.fire({
