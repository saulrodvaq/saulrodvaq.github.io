...
if (!esInstructor($_SESSION['usuario'])) {
    echo "No estás autorizado para acceder a esta página.";
    exit;
}

function esInstructor($usuario) {
    $servername = "localhost";
    $username = "root";
    $password = "";
    $dbname = "dual_bd";
    $conn = mysqli_connect($servername, $username, $password, $dbname);
    $query = "SELECT * FROM instructores WHERE usuario = '$usuario'";
    $resultado = mysqli_query($conn, $query);
    $esInstructor = mysqli_num_rows($resultado) > 0;
    mysqli_close($conn);
    return $esInstructor;
}
...
$sql = "SELECT *, idInstructor FROM instructores WHERE usuario = '$usuario'";
...
// Aquí agregamos el idDireccion a la sesión
$_SESSION['idInstructor'] = $usuario['idInstructor'];
                        ...
                        <div class="container-fluid">
                            <div class="table-responsive text-center table-hover">
                                <table class="table table-responsive-sm text-center table-hover" id="tabla_id">
                                    ...
                                    <tbody>
                                        <?php
$instructorActual = $_SESSION['idInstructor'];
include("templates/conexion.php");
...
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          WHERE r.estatus_instructor = 'sin_revisar' AND i.idInstructor = '$instructorActual'
...
while ($registro = mysqli_fetch_array($respuesta)) {
    ...
    echo "<td>";
    echo "<button type='button' class='vobo-button efectoboton edit1' data-id='{$registro['idReporte']}' data-retro='{$registro['retro_instructor']}' data-estatus='{$registro['estatus_instructor']}' onclick='updateStatus(this)'>Revisión</button>";
    echo "</td>";
    echo "</tr>";
}
...
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          INNER JOIN tutores t ON r.idTutor = t.idTutor
          INNER JOIN instructores i ON r.idInstructor = i.idInstructor
          WHERE r.estatus_instructor = 'no_aprobado' AND i.idInstructor = '$instructorActual'
          ORDER BY r.idReporte DESC";
...
while ($registro = mysqli_fetch_array($respuesta)) {
    ...
    echo "<td>{$registro['apellidosinstructor']} {$registro['nombresinstructor']}</td>";
                ...
                >";
                    echo "<button type='button' class='vobo-button efectoboton edit2' data-id='{$registro['idReporte']}' data-retro='{$registro['retro_instructor']}' data-estatus='{$registro['estatus_instructor']}' onclick='updateStatus(this)'>Revisión</button>";
                    echo "<button type='button' style='margin-top: 3px;' class='retro-button efectoboton edit2' data-id='{$registro['idReporte']}' data-retro='{$registro['retro_instructor']}' data-estatus='{$registro['estatus_instructor']}' onclick='showFeedback(this)'>Retroalimentación</button>";
                    echo "</td>";
                    echo "</tr>";
                }
                ?>
...
$query = "SELECT r.*, t.nombres as nombrestutor, t.apellidos as apellidostutor, i.nombres as nombresinstructor, i.apellidos as apellidosinstructor
          ...
          INNER JOIN tutores t ON r.idTutor = t.idTutor
          INNER JOIN instructores i ON r.idInstructor = i.idInstructor
          WHERE r.estatus_instructor = 'aprobado' AND i.idInstructor = '$instructorActual'
          ORDER BY r.idReporte DESC";
...
while ($registro = mysqli_fetch_array($respuesta)) {
    ...
    echo "<td>{$registro['apellidosinstructor']} {$registro['nombresinstructor']}</td>";
                    ...
                    echo "<td>";
                    echo "<button type='button' class='retro-button efectoboton edit4' data-id='{$registro['idReporte']}' data-retro='{$registro['retro_instructor']}' data-estatus='{$registro['estatus_instructor']}' onclick='showFeedback(this)'>Retroalimentación</button>";
                    echo "</td>";
                    echo "</tr>";
                }
                ?>
        ...
        Swal.fire({
            ...
            inputValidator: (value) => {
                Swal.fire({
                        ...
                        cancelButtonText: 'Cancelar',
                        preConfirm: (text) => {
                            return fetch("gestion/guardar_retro_instructor.php", {
                                    method: "POST",
                                    body: JSON.stringify({
                                        id: id,
                                        retro: text,
